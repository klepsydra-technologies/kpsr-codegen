...
TEST(KpsrDdsCodegenTest5, compositionTypeRelatedMapperTest) {
    ...
    fromDDSProvider.registerToTopic("kpsr_dds_codegen_test_topic5", &dataReader, true, basicProvider.getPublisher());
    ...
    fromDDSProvider.unregisterFromTopic("kpsr_dds_codegen_test_topic5", &dataReader);
}
